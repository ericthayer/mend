...
1. **Strict Backlog Order:** Only select tasks marked `READY` whose dependencies are marked `DONE` in `docs/IMPLEMENTATION_LOG.md`. Work on one task at a time.
...
6. **Execution Harness:**
   ...
   * Run `npm run lint`, `npm run typecheck`, `npm test:run`, and `npm run build` after changes.
